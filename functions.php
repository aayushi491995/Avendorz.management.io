...
function stsblogfeedly_setup() {
	...
	register_nav_menus(
			array(
				'primary' => __( 'Navigation Menu', 'blogfeedly' ),
				'footer' => __( 'Footer Menu', 'blogfeedly' ),
				'social' => __( 'Social Links Menu', 'blogfeedly' ),
			)
		);
...
function stsblogfeedly_scripts_styles() {
	...
	wp_enqueue_style( 'stsblogfeedly-fonts', get_template_directory_uri() . '/fonts/avenir/font.css');
	wp_enqueue_style( 'stsblogfeedly-google-fonts', 'https://fonts.googleapis.com/css2?family=Source+Sans+Pro:ital,wght@0,300;0,400;0,600;0,700;0,900;1,300;1,400;1,600;1,700;1,900&display=swap', false );

	// Load the main stylesheet.
	wp_enqueue_style( 'stsblogfeedly-style', get_stylesheet_uri() );
	wp_enqueue_style( 'stsblogfeedly-custom', get_template_directory_uri() . '/css/custom.css');
	...
	<div class="wrap about-wrap blogfeedly-add-css">
		...
		<table class="wp-list-table widefat">
			...
			<tbody>
				...
				<tr>
					<td><?php echo __("0.2 Seconds Load Time", "blogfeedly"); ?></td>
					<td><span class="cross"><img src="<?php echo esc_url( get_template_directory_uri() . '/icons/cross.png' ); ?>" alt="<?php echo __("No", "blogfeedly"); ?>" /></span></td>
					<td><span class="checkmark"><img src="<?php echo esc_url( get_template_directory_uri() . '/icons/check.png' ); ?>" alt="<?php echo __("Yes", "blogfeedly"); ?>" /></span></td>
				</tr>
				<tr>
...
//customeize placeholder text for search form
function blogfeedly_search_form( $html) {
        $html = str_replace( 'placeholder="Search ', 'placeholder="Search the knowledge base?', $html);
        return $html;
}
add_filter( 'get_search_form', 'blogfeedly_search_form' );

//remove URL from comments
function blogfeedly_remove_comment_url($arg) {
    $arg['url'] = '';
    return $arg;
}
add_filter('comment_form_default_fields', 'blogfeedly_remove_comment_url');

//remove cookie consent from comments
function blogfeedly_remove_cookie_consent( $fields ) {
	unset( $fields['cookies'] );
	return $fields;
}
add_filter( 'comment_form_default_fields', 'blogfeedly_remove_cookie_consent' );

//change text to leave a reply on comment form
function blogfeedly_comment_reply_text ($arg) {
	$arg['title_reply'] = __('Share. Ask.');
	return $arg;
}
add_filter('comment_form_defaults','blogfeedly_comment_reply_text');

//remove form comments notes
function blogfeedly_remove_comment_notes( $arg ) {
	$arg['comment_notes_before'] = '';
	return $arg;
}
add_filter( 'comment_form_defaults', 'blogfeedly_remove_comment_notes' );

//customeize placeholder text for comment form
function blogfeedly_comment_form($arg) {
	$arg['comment_field'] = '<p class="comment-form-comment"><textarea id="comment" name="comment" placeholder="Write Comment" cols="45" rows="8" maxlength="65525" required="required" aria-required="true"></textarea></p>';
    return $arg;
}
add_filter('comment_form_defaults', 'blogfeedly_comment_form');

//customeize placeholder text for comment form
function blogfeedly_comment_form_fields($arg) {
	$arg['author'] = '<p class="comment-form-author"><input id="author" name="author" placeholder="Name" type="text" value="" size="30" maxlength="245" required="required"></p>';
	$arg['email'] = '<p class="comment-form-email"><input id="email" name="email" placeholder="Email Address" type="email" value="" size="30" maxlength="100" required="required"></p>';
    return $arg;
}
add_filter('comment_form_default_fields', 'blogfeedly_comment_form_fields');

//add subscribe form after content
//https://wpdevelopers.com/adding-content-before-and-after-the_content/
function wpdev_before_after($content) {
    if(is_page() || is_single()) {
        $beforecontent = '';
    	$aftercontent = '<div class="subscribe-container">[contact-form-7 id="110" title="MasterClass"]</div>';
